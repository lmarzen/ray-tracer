...
int main(int argc, char *argv[])
{
  ...
  unsigned int bucket_map[num_regions]; // only used by root thread
  ...
  { // root thread is central communicator and will manage assigning each thread
    ...
    { // the first comm_sz buckets are assigned to the first comm_sz threads
      bucket_map[next_region] = next_region + 1;
    }
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  // root thread must collect and combine the results from all other threads.
  // recieving UINT_MAX from root will indicate that the collection is finished.
  if (my_rank != 0)
  {
  ...
  {
    unsigned char *recv_buffer = calloc(bucket_size, sizeof(unsigned char));
    unsigned int bucket_index = 0;
    for (unsigned int i = 0; i < num_regions; ++i)
    {
      MPI_Send(&bucket_index, 1, MPI_INT, bucket_map[i], 0, MPI_COMM_WORLD);
      MPI_Recv(recv_buffer, bucket_size, MPI_CHAR, bucket_map[i], 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      memcpy(framebuffer + bucket_index, recv_buffer, bucket_size);
      bucket_index += bucket_size;
    }

    for (unsigned int i = 1; i < comm_sz; ++i)
    { // send UINT_MAX to all threads to indicate that collection is done.
